...
def get_departure_time():
    try:
        ...
        # Fetch train for capacity
        train = trains_collection.find_one({"_id": schedule["train_id"]})
        if not train:
            return jsonify({"error": "Train not found"}), 404
        
        # Fetch route for name
        route = routes_collection.find_one({"_id": ObjectId(route_id)})  # Add this line
        if not route:
            return jsonify({"error": "Route not found"}), 404

        # SAFE capacity extraction
        capacity = train.get("capacity")
        if isinstance(capacity, dict):  # BSON format
            total_seats = int(capacity.get("$numberInt", 10))  # Default to 10 if missing
        elif isinstance(capacity, int):  # Plain integer
        ...
        return jsonify({
            ...
            "train_name": train["train_name"],
            "route_id": route_id,
            "route_name": route["route_name"],  # Add this line
            "departure_time": f"{departure_date} {schedule['departure_time']}",
            "arrival_time": f"{departure_date} {schedule['arrival_time']}",  # Add this line
...
def get_routes_detail():
    routes = list(routes_collection.find({}, {"_id": 1, "route_name": 1, "departure": 1, "arrival": 1}))
